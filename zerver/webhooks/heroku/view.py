...
def api_heroku_webhook(
    ...
    message_body_function = EVENT_FUNCTION_MAPPER.get(resource)
    if message_body_function is None:
        raise UnsupportedWebhookEventTypeError(resource)

    message = message_body_function(payload, actor, formatted_action, resource)
